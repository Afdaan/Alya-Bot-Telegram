        ...
        if update.message.reply_to_message:
            ...
            if replied.from_user and replied.from_user.is_bot:
                if replied.from_user.id == context.bot.id:
                    ...
                    # Check if the replied message is part of a conversation
                    if "Generated by" not in (replied.text or ""):
                        replied_message_is_conversation = True

        # In groups, only respond if mentioned or replying to Alya
        if update.message.chat.type in ["group", "supergroup"]:
            is_mentioned = f"@{context.bot.username}" in message_text
            if not is_mentioned and not is_reply_to_alya:
                return
